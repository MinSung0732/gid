...
assert.match(moduleSource, /showAbsorbLoss/);
...

{
    ...
    harness = createHarness({
      ...
      engineOverrides: {
        discardFromHand(currentRun, index) {
          ...
          currentRun._absorbFeedback = 4;
          currentRun.hp -= 3;
          currentRun.battle.shield += 2;
          return true;
        },
      },
    });
  ...
  const discardIndex = harness.events.findIndex(([name]) => name === "discard-animation"),
    damageIndex = harness.events.findIndex(([name, amount]) => name === "player-damage" && amount === 3),
    shieldIndex = harness.events.findIndex(([name, amount]) => name === "shield" && amount === 2),
    absorbIndex = harness.events.findIndex(([name, amount]) => name === "absorb" && amount === 4);
  assert.ok(discardIndex >= 0 && damageIndex > discardIndex && shieldIndex > damageIndex && absorbIndex > shieldIndex);
