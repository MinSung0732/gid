...
assert.match(frame, /analyzeBuild as analyzeSharedBuild/, "PC frame imports the shared build analyzer");
assert.match(data, /handLimit:\s*Object\.freeze\(\{[\s\S]*?label:\s*"손패 한도"[\s\S]*?description:/);
assert.match(data, /deckLimit:\s*Object\.freeze\(\{[\s\S]*?label:\s*"덱 한도"[\s\S]*?description:/);
assert.match(frame, /\["▤",\s*"손패 한도",\s*`\$\{E\.handLimit\(run\)\}장`,\s*"handLimit"\]/);
assert.match(frame, /\["▦",\s*"덱 한도",\s*`\$\{run\.deck\.length\} \/ \$\{E\.deckLimit\(run\)\}장`,\s*"deckLimit"\]/);
...
const deckLimitIndex = frame.indexOf('["▦", "덱 한도"');
