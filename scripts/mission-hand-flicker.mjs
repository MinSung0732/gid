...
for (const needle of ["after-engine-play", "after-collapse", "after-render", "after-stage-draw"])
  if (!source.includes(needle)) throw new Error(`instrumentation failed: ${needle}`);
await fs.writeFile(orchestratorPath, source);

const cards = Object.values(CARDS);
const simple = (card) =>
  card && card.id !== "impurity" && !card.requiredAbsorb && !card.absorbCost &&
  !card.executeRatio && !card.randomEachHit && !card.searchDrawCard &&
  !card.discard && !card.pendingDiscard;
const chosen = {
  contact: cards.find((card) => simple(card) && card.tier === 1 && card.attack > 0 && (card.attackPattern || "contact") === "contact" && !card.draw && !card.drawOnKill && !card.drawOnBreak),
  nonContact: cards.find((card) => simple(card) && card.tier === 1 && card.attack > 0 && card.attackPattern === "nonContact" && !card.draw && !card.drawOnKill && !card.drawOnBreak),
  draw: cards.find((card) => simple(card) && card.tier === 1 && card.draw > 0 && !card.attack && !card.burst && !card.weight && !card.randomDiscard),
  defense: cards.find((card) => simple(card) && card.tier === 1 && card.shield > 0 && !card.draw && !card.attack),
  heal: cards.find((card) => simple(card) && card.tier === 1 && card.heal > 0 && !card.draw && !card.attack),
  randomDiscard: cards.find((card) => card.id !== "impurity" && card.randomDiscard > 0),
  impurity: CARDS.impurity,
};
for (const [kind, card] of Object.entries(chosen)) if (!card) throw new Error(`missing ${kind} card`);
console.log("CHOSEN_CARDS", JSON.stringify(Object.fromEntries(Object.entries(chosen).map(([kind, card]) => [kind, { id: card.id, name: card.name, draw: card.draw || 0, randomDiscard: card.randomDiscard || 0, attackPattern: card.attackPattern || null }])), null, 2));

const fillers = cards.filter((card) => card.id !== "impurity" && simple(card)).slice(0, 20).map((card) => card.id);
const makeHand = (first, size) => [first, ...Array.from({ length: Math.max(0, size - 1) }, (_, index) => fillers[index % fillers.length])];
const drawPile = Array.from({ length: 10 }, (_, index) => fillers[(index + 8) % fillers.length]);
const scenarios = [
...
try {
  ...
  for (const mode of modes) {
    ...
    page.on("console", (message) => {
      ...
      if (!text.startsWith("__HAND_PROBE__ ")) return;
      probes.push(JSON.parse(text.slice("__HAND_PROBE__ ".length)));
    });
    ...
    for (const scenario of scenarios) {
      ...
      await page.evaluate(({ baseline, desiredHand, drawPile, damaged }) => {
        ...
        const prefix = baseline.key.slice(0, -"gyeolideun-harmony-save-v2".length);
        localStorage.setItem(baseline.key, encoded);
        localStorage.setItem(`${prefix}gyeolideun-harmony-save-v2-temporary`, encoded);
        localStorage.setItem(`${prefix}gyeolideun-harmony-save-v2-backup`, encoded);
      }, { baseline, desiredHand, drawPile, damaged: Boolean(scenario.damaged) });

      await page.reload({ waitUntil: "networkidle" });
      const resume = page.locator('[data-action="resume"]');
      if (await resume.count()) await resume.click();
      await page.waitForSelector(".hand > .card");
      const before = await page.evaluate(({ caseName, expectedCount }) => {
      ...
      }, { caseName, expectedCount: scenario.size });
      if (before.domCount !== scenario.size) throw new Error(`${caseName}: hand injection expected ${scenario.size}, got ${before.domCount}`);
      ...
      await page.locator('.hand > .card[data-index="0"]').click({ timeout: 5000 });
      ...
      const result = {
        caseName,
        mode,
        card: { id: scenario.first, name: CARDS[scenario.first].name },
        before,
        probes: caseProbes,
        frames,
        summary: {
      ...
      results.push(result);
      console.log("CASE_SUMMARY", caseName, JSON.stringify(result.summary));
    }
    await context.close();
  }

  await fs.writeFile(`${outDir}/hand-flicker-results.json`, JSON.stringify({ chosen: Object.fromEntries(Object.entries(chosen).map(([kind, card]) => [kind, { id: card.id, name: card.name }])), results }, null, 2));
  console.log("RESULT_SUMMARIES", JSON.stringify(results.map(({ caseName, summary }) => ({ caseName, ...summary })), null, 2));
  await browser.close();
} finally {
  server.kill("SIGTERM");
