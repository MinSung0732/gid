...
assert.match(lateUiFix, /height:\s*64px\s*!important/, "enemy ART stage should keep a stable fixed height after the three information blocks");
...
assert.match(lateUiFix, /> \.intent-wrap[\s\S]*?grid-row:\s*1\s*!important/, "next action is the first independent information block");
assert.match(lateUiFix, /> \.enemy-status-panel[\s\S]*?grid-row:\s*2\s*!important/, "current statuses are the second independent information block");
assert.match(lateUiFix, /> \.late-enemy-summary[\s\S]*?grid-row:\s*3\s*!important/, "pattern/mechanic is the third independent information block");
assert.match(lateUiFix, /> \.enemy-status-panel[\s\S]*?height:\s*47px\s*!important/, "status block stays visibly sized even in crowded encounters");
assert.match(lateUiFix, /> \.late-enemy-summary[\s\S]*?height:\s*47px\s*!important/, "pattern/mechanic block is visually distinct rather than a tiny line");
assert.match(lateUiFix, /> \.intent-wrap > \.intent > \.intent-copy > small[\s\S]*?display:\s*none\s*!important/, "status/detail prose is not rendered as tiny text under the action name");
assert.match(lateUiFix, /> \.intent-wrap > \.intent > \.intent-value[\s\S]*?min-width:\s*max-content\s*!important/, "intent damage unit remains visible while action copy shrinks first");
assert.match(lateUiFix, /> \.attack-warning[\s\S]*?grid-row:\s*4\s*!important/, "attack warning overlays the ART row below all three information blocks");
assert.match(lateUiFix, /> \.enemy-hp[\s\S]*?grid-row:\s*6\s*!important/, "HP bar follows the ART/name rows after the information stack");
assert.match(lateUiFix, /> \.enemy-vitals[\s\S]*?grid-row:\s*7\s*!important/, "remaining HP value follows the restored three-block information stack");
assert.match(lateUiFix, /> \.late-enemy-telemetry[\s\S]*?display:\s*none\s*!important/, "expanded duplicate mechanic telemetry must stay off the ART stage");

assert.match(lateUiSource, /function combinedSummaryParts\(/, "pattern and mechanic are rendered as compact entries inside their dedicated block");
assert.match(lateUiSource, /label\.textContent = "패턴 \/ 기믹"/, "third block has a visible pattern/mechanic heading");
assert.match(lateUiSource, /late-summary-chip late-summary-\$\{part\.kind\}/, "pattern and mechanic use distinct chips within the third block");
assert.match(lateUiSource, /empty\.textContent = "정보 없음"/, "third block remains structurally visible when no late information exists");
assert.match(lateUiSource, /enemy-status-panel/, "late summary is anchored after the dedicated status block");
assert.doesNotMatch(lateUiSource, /label\.textContent = "패턴 예고"/, "separate legacy pattern tile is not recreated");
assert.doesNotMatch(lateUiSource, /button\.className = "late-mechanic-button"/, "separate legacy mechanic tile is not recreated");
assert.doesNotMatch(lateUiSource, /text:\s*`가시 /, "Thorns should only appear in current-status UI, not duplicated in late telemetry");
assert.match(lateUiSource, /function relationMarkers\(/, "relationship mechanics still generate target-side markers");
assert.match(lateUiFix, /> \.late-relation-markers[\s\S]*?grid-row:\s*4\s*!important/, "relation information stays on the ART row without consuming another information block");
assert.match(mainUiSource, /hits > 1[\s\S]*?총 예상 피해/, "multi-hit enemy intent keeps its hit-count calculation");
assert.match(mainUiSource, /damage = perHit \* hits/, "strong-attack warning still uses total multi-hit damage");
assert.match(mainUiSource, /\$\{perHit\} × \$\{hits\}/, "multi-hit enemy intent keeps per-hit damage multiplied by hit count");
assert.doesNotMatch(mainUiSource, /class="intent-heading"/, "current status UI is no longer embedded in the next-action heading");
