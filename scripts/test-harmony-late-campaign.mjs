...
assert.match(lateUiFix, /> \.intent-wrap[\s\S]*?grid-row:\s*1\s*!important/, "current action should remain the primary top row");
assert.match(lateUiFix, /> \.late-enemy-summary[\s\S]*?grid-row:\s*2\s*!important/, "pattern and mechanic should share one optional summary row");
assert.match(lateUiFix, /> \.late-enemy-summary[\s\S]*?height:\s*19px\s*!important/, "combined pattern/mechanic summary must stay one line tall");
assert.match(lateUiFix, /> \.late-enemy-summary[\s\S]*?text-overflow:\s*ellipsis\s*!important/, "long pattern/mechanic summary should ellipsize instead of growing vertically");
assert.match(lateUiFix, /> \.intent-wrap > \.intent-heading[\s\S]*?justify-content:\s*space-between\s*!important/, "status badges should share the next-action heading row");
assert.match(lateUiFix, /\.enemies-field\.enemies-3[\s\S]*?> \.intent-copy > small[\s\S]*?display:\s*none\s*!important/, "three-enemy layout should shorten secondary intent prose before shrinking core values");
assert.match(lateUiFix, /> \.intent-wrap > \.intent > \.intent-value[\s\S]*?min-width:\s*max-content\s*!important/, "intent damage unit should remain visible while copy shrinks first");
...
assert.match(lateUiSource, /function combinedSummary\(/, "pattern and mechanic should be composed into one summary line");
assert.match(lateUiSource, /`패턴 \$\{preview\.text\}`/, "combined summary should identify pattern information");
assert.match(lateUiSource, /`기믹 \$\{compactMechanicLabel\(rows\)\}`/, "combined summary should identify mechanic information");
assert.match(lateUiSource, /className = "late-enemy-summary"/, "late UI should render one combined summary control");
assert.doesNotMatch(lateUiSource, /label\.textContent = "패턴 예고"/, "separate pattern tile should no longer be created");
assert.doesNotMatch(lateUiSource, /button\.className = "late-mechanic-button"/, "separate mechanic tile should no longer be created");
assert.doesNotMatch(lateUiSource, /text:\s*`가시 /, "Thorns should only appear in normal status UI, not duplicated in late telemetry");
