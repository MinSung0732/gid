...
assert.match(lateUiFix, /\.intent-wrap,[\s\S]*?\.late-pattern-preview,[\s\S]*?\.late-mechanic-button[\s\S]*?height:\s*51px\s*!important/, "top modules should share the same fixed height");
assert.match(lateUiFix, /> \.late-mechanic-button > b[\s\S]*?height:\s*40px\s*!important/, "mechanic body should match the 40px action/pattern body height");
