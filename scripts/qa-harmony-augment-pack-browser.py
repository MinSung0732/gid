...
try:
    ...
    while time.time() < deadline:
        result = driver.execute_script(
            "return window.__augmentPackBrowserSmoke || null"
        )
        if result:
            break
        time.sleep(0.1)
    ...
    if result.get("pass") is not True or missing:
        raise AssertionError(
            {"result": result, "missing_or_failed": missing}
        )
