...
import * as S from "./statuses.js?v=20260922-concentration-1";
          ...
          resolveCardDirectDamage(
            ...
            ({ result, hitEnemy: logicalEnemy, logicalIndex, logicalTotal }) => {
              ...
              if (
                ...
                c.chanceStatusOnHit &&
                landed &&
                random(s) < c.chanceStatusOnHit.chance
              )
                applyBattleStatus(
                  s,
                  "enemy",
              ...
              if (
                ...
                enemy.hp > 0 &&
                landed &&
                logicalIndex === logicalTotal - 1
              ) {
                const totalWeak = Math.min(5, Math.max(1, c.weakOnHit)),
                  amount =
                    Math.floor(((hit + 1) * totalWeak) / hits) -
                    Math.floor((hit * totalWeak) / hits);
                if (amount > 0)
