...
- 추가 피해/보너스 피해의 전투 피드백이 유물·특성 출처 메타데이터(sourceType/sourceId/parentSource/sourceMetadata)를 끝까지 유지하도록 보강하고, 오른쪽 증강 목록의 sourceId lookup을 통해 해당 유물·특성 Trigger Focus가 실행되도록 수정했다. 특정 유물 ID 하드코딩 없이 공통 효과 source metadata 경로를 사용한다.
