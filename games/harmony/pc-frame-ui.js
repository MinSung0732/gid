  ...
  return `<section class="run-build-items-section"><div class="run-build-subhead"><span>${title}</span><b>${rows.length}</b></div>${rows.length
    ? rows
        .map(({ id, item, count }) => {
          ...
          return `<div class="run-build-item tier-mark-${item.tier}" data-augment-id="${escapeHtml(id)}"><i aria-hidden="true">${icon}</i><div><strong>${escapeHtml(item.name)}${count > 1 ? ` ×${count}` : ""}</strong><small><em>${escapeHtml(rarity)}</em>${item.description ? ` · ${escapeHtml(item.description)}` : ""}</small></div></div>`;
        })
        .join("")
