...
function lobby() {
  const hasActiveRun = Boolean(run && !run.finished),
    resumeAction = hasActiveRun
      ? '<button class="primary lobby-action-primary" data-action="resume">이전 여정 이어하기 →</button>'
      : "",
    newAction = `<button class="${hasActiveRun ? "lobby-action-secondary" : "primary lobby-action-primary"}" data-action="new">새로운 조향 시작${hasActiveRun ? "" : " →"}</button>`,
...
function battle() {
  const b = run.battle,
    ...
    intentDisplay = (enemy) => {
      ...
      for (const statuses of [intent.applySelf, intent.applyAllies]) {
        const text = intentStatuses(statuses);
