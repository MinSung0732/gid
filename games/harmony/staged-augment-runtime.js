...
function withTemporaryCardValues(cards, card, overrides, callback) {
  const definition = cards[card.id];
  if (!definition || !Object.keys(overrides).length) return callback();
  const saved = [];
...
function uniqueHitTargetIndexes(state, pattern) {
  const hits = state?._enemyHitFeedback || [];
  return [...new Set(hits
...
function applyPostNonContactResonance(Core, state, snapshot) {
  ...
  const targets = uniqueHitTargetIndexes(state, "nonContact")
...
function applyResonanceBuffer(Core, state, snapshot) {
  ...
  const hits = (state._enemyHitFeedback || []).filter((hit) => !hit.statusId && (hit.damage > 0 || hit.blocked > 0));
...
function prepareCard(Core, cards, state, card, definition, snapshot) {
  ...
  if (consumedResonance >= 6 && Core.power(state, "resonanceConsumeRefundDraw") > 0 && !battle._stagedCriticalDischarge) {
  ...
  return { overrides, consumedResonance, chainSplash, criticalDischarge };
}

function applyChainSplashPre(Core, state, snapshot, prepared) {
  const splash = prepared.chainSplash;
  if (!splash || splash.amount <= 0) return;
  const battle = state.battle;
  for (let index = 0; index < battle.enemies.length; index++) {
    const enemy = battle.enemies[index];
    if (!enemy || enemy === splash.mainTarget || enemy.hp <= 0) continue;
    const before = enemy.hp;
    const dealt = Math.min(enemy.hp, Math.max(0, Math.round(splash.amount)));
    enemy.hp -= dealt;
    state._enemyHitFeedback ??= [];
    state._enemyHitFeedback.push({
      targetIndex: index,
      damage: dealt,
      blocked: 0,
    ...
    if (before > 0 && enemy.hp <= 0) enemy.hp = 0;
  }
}

function finishChainSplash(state, prepared) {
  const splash = prepared.chainSplash;
  if (!splash || !state?.battle || splash.resonanceAdd <= 0) return;
...
export function playWithStagedAugments(Core, cards, items, state, index, meta) {
  ...
  const prepared = prepareCard(Core, cards, state, card, definition, snapshot);
  // Chain Collapse splash is staged before Core.play so Core's existing victory/reward
  // resolution still observes enemies killed by the splash. The public card outcome is
  // unchanged; this is deliberately isolated here until engine-core exposes a damage hook.
  applyChainSplashPre(Core, state, snapshot, prepared);

  const pendingDiscardBefore = battle.pendingDiscard || 0;
  const result = withTemporaryCardValues(cards, card, prepared.overrides, () => Core.play(state, index, meta));
  if (!result) return result;
  const controlFailed = ["confusion", "interference"].includes(state._controlFeedback?.statusId);
  const definitionAfter = cards[card.id];
  ...
  if (!controlFailed) {
    ...
    if (prepared.criticalDischarge && !battle._stagedCriticalDischarge) {
      Core.gainCurrentAp(state, 1);
      battle._stagedCriticalDischarge = true;
    }
  }

  if ((battle.pendingDiscard || 0) > pendingDiscardBefore && definitionAfter.stagedDiscardRefundBaseCost)
    battle._stagedDiscardRefundQueue += (battle.pendingDiscard || 0) - pendingDiscardBefore;

  if (attackCard && pattern === "contact" && paidCost > 0 && !controlFailed)
    battle._stagedPaidContactCardsPlayed += 1;

  finishChainSplash(state, prepared);
