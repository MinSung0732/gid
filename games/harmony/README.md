...
카드 설명과 HTML 표현, 상태 라벨, 티어 표기, 전투 중 상태 보정 표시는 `card-presentation.js`가 공통 관리합니다. 게임 규칙과 실제 카드 효과는 엔진/데이터 모듈이 계속 소유합니다.
...
- `special-deck-picker-ui.js`: 실험실 노트 재지정, 카드 제거, 피 제단 정화, 거울 복제 등 덱에서 대상을 고르는 특수 방 흐름
- `rest-upgrade-ui.js`: 휴식방 회복/강화 UI, 강화 전후 비교, 변경 수치 강조, hover/focus 비교
- `starting-deck-builder-ui.js`: 일반/테스트 시작 덱 선택, 카테고리/필터, 검증
- `deck-replacement-ui.js`: 덱 한도에 도달한 카드 보상의 교체 선택

최근 UI 정리에서는 저주 선택/버리기 액션 표시를 맞추고, 카드 사용 재생용 clone이 손패 카드의 표현을 유지하도록 보정했습니다.
...
Harmony는 초기의 큰 `main.js` / `engine.js` 중심 구조에서 기능 경계를 나누는 방향으로 단계적으로 모듈화되었습니다. 최근 Phase 1~6 리팩터링의 기본 원칙은 게임 규칙을 무분별하게 바꾸는 것이 아니라 책임과 테스트 경계를 명확하게 만드는 것입니다.
...
- `statuses.js`: 상태이상 정의, 지속/소비 규칙과 공통 처리
...
- `card-presentation.js`: 카드 효과 문구, 상세/요약 markup, 카드 HTML, 상태 라벨과 전투 보정 표시
...
- `deck-replacement-ui.js`: 가득 찬 덱의 카드 보상 교체 UI
- `special-deck-picker-ui.js`: 특수 방의 덱 대상 선택 UI
- `rest-upgrade-ui.js`: 휴식방 카드 강화/비교 UI
...
- `status-proc-vfx.css`: 출혈/연소 등 상태 발동 피드백 스타일
...
- `sound-runtime.js`: SFX storage/Audio 생성 runtime 경계
- `sound.js`: 효과음 매핑과 재생 정책
...
`main.js`와 `engine.js`의 많은 책임이 별도 모듈로 이동했으며, 이후 구조 변경도 기능 단위로 분리하고 회귀 테스트와 함께 진행하는 것을 원칙으로 합니다.
...
- 상태이상 규칙/콘텐츠/발동 VFX: `test-harmony-status-overhaul.mjs`, `test-harmony-status-content.mjs`, `test-harmony-status-proc-vfx.mjs`
...
- 저장/browser/sound runtime 경계: `test-harmony-persistence-runtime.mjs`, `test-harmony-browser-runtime.mjs`, `test-harmony-sound-runtime.mjs`
- 사운드 동작: `test-harmony-sound.mjs`
...
- 덱 교체/특수 방 덱 선택/휴식 강화/카드 표현: 각 전용 UI·presentation 테스트
- 전투 턴/카드/게임 액션 오케스트레이션: 각 orchestrator 전용 테스트
- 데스크톱 프레임: `test-harmony-pc-frame-layout.mjs`

최근 완료된 Harmony 리팩터링 및 UI 수정 PR은 `npm run check:harmony`, `npm run test:harmony`, `npm test`를 통과한 상태로 병합된 기록이 있습니다. `package.json`의 현재 전체 `npm run check`도 Harmony 검사까지 연결되도록 구성되어 있습니다.
...
- 상태이상 체계는 발동 소비형/시간형/지속형 역할을 구분하는 현재 규칙으로 정리되어 있습니다.
...
- 전투 VFX/UI와 도감/보상/런 요약/시작 덱/덱 교체/특수 방/휴식 UI가 별도 모듈로 분리되어 있습니다.
- 카드 표현 책임은 `card-presentation.js`로 분리되어 있습니다.
- 전투 턴, 카드 사용, 일반 게임 액션 실행 순서가 오케스트레이터로 분리되어 있습니다.
- 저장, 브라우저 플랫폼, 사운드의 직접 runtime 접근은 각각 전용 경계 모듈로 분리되어 있습니다.
