...
async function toFile(canvas, name) {
  ...
  try {
    blob = await new Promise((resolve, reject) => {
      try { canvas.toBlob(value => value ? resolve(value) : reject(new Error('empty blob')), 'image/png'); }
...
export async function createChallengeCard(record) {
  ...
  context.fillStyle = '#235347'; context.fillRect(0, 0, 1080, 22);
  context.fillStyle = '#235347'; context.font = '700 38px sans-serif'; context.fillText('GYEOLIDEUN · SCENT MEMORY', 70, 115);
  context.fillStyle = '#333331'; context.font = '700 64px serif'; context.fillText('향기의 기억 챌린지', 70, 230);
  context.fillStyle = '#F3E9DC'; context.fillRect(70, 310, 940, 430);
  context.fillStyle = '#235347'; context.font = '28px sans-serif'; context.fillText('최고 점수', 130, 410);
  context.font = '700 92px sans-serif'; context.fillText(`${record.score.toLocaleString('ko-KR')}점`, 130, 530);
  context.fillStyle = '#BD4860'; context.font = '28px sans-serif'; context.fillText('최고 도달 레벨', 130, 625);
  context.font = '700 64px sans-serif'; context.fillText(`LEVEL ${record.level}`, 130, 700);
  context.fillStyle = '#333331'; context.font = '30px sans-serif'; context.fillText('목숨 3개로 당신의 기억력에 도전해보세요.', 70, 855);
  context.fillStyle = '#235347'; context.font = '24px sans-serif'; context.fillText('향기는 사라져도 좋은 기억은 오래 머물러요.', 70, 970);
...
function download(file) {
  ...
  anchor.href = url; anchor.download = file.name || 'gyeolideun-memory-challenge.png';
...
export async function shareChallengeRecord(record) {
  ...
  const text = `결이든 향기의 기억 챌린지\n최고 ${record.score.toLocaleString('ko-KR')}점 · LEVEL ${record.level}`;
  if (navigator.share && navigator.canShare?.({ files: [file] })) {
    try {
      await navigator.share({ files: [file], title: '향기의 기억 챌린지', text });
      return '공유할 앱을 선택해 주세요.';
    } catch (error) { if (error.name === 'AbortError') throw error; }
  }
  download(file);
  return '공유 창을 열 수 없어 최고 기록 이미지를 저장했어요.';
}
...
export async function shareChallengeLink(record) {
  ...
  if (navigator.share) {
    try {
      await navigator.share({ title: '향기의 기억 챌린지', text: value, url });
      return '기록 링크를 공유했어요.';
    } catch (error) { if (error.name === 'AbortError') throw error; }
  }
  try { await navigator.clipboard.writeText(value); }
  catch { copyText(value); }
  return '기록과 게임 링크를 복사했어요.';
}
