...
export function setupSocialShare({imageButton,status,canvas,getResult}) {
  imageButton.addEventListener('click',async()=>{const result=getResult(),url=scoreResultUrl(result);try{const file=createSocialCardFile(result,canvas);if(navigator.share&&navigator.canShare?.({files:[file]})){await navigator.share({files:[file],title:'결이든 향기 기록',text:`${scoreShareText(result)}\n${url}`});status.textContent='공유할 앱을 선택해 주세요.';}else{downloadFile(file);status.textContent='공유용 이미지를 저장했어요. 원하는 앱에서 이미지를 선택해 주세요.';}}catch(error){status.textContent=error.name==='AbortError'?'공유를 취소했어요.':'공유 이미지를 만들지 못했어요. 다시 시도해 주세요.';}});
}
